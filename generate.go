...
func (swagger Swagger) GenerateDocs() (jsonDocs []byte) {
	...
	for _, endpoint := range endpoints {
		...
		if endpoint.Errors != nil {
			for k, v := range endpoint.Errors {
				errorSchema := &swaggerResponseScheme{
					Ref: fmt.Sprintf("#/definitions/%T", v.Model),
				}

				responses[k] = swaggerResponse{
...
func generateSwaggerDefinition(swagger *Swagger, endpoints []Endpoint) {
	...
	for _, endpoint := range endpoints {
		...
		if endpoint.Errors != nil {
			createErrorDefinition(swagger, endpoint.Errors)
		}
	}
}

func createErrorDefinition(swagger *Swagger, t map[string]ErrorSchema) {
	for _, k := range t {
		createdefinition(swagger, k.Model)
	}
}
...
func createdefinition(swagger *Swagger, t interface{}) {
	...
	if reflectReturn.Kind() == reflect.Map {
